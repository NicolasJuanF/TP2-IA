        ...
        for(Nodo n : nodosAdyacentes){
    		if((nodoActual.getX()==n.getX())&&(nodoActual.getY()>n.getY())&&(!n.equal(agState.getUltimaPosicionVisitada()))){
				...
				if((!estaBloqueado)&&(agState.getenergiaConsumida()<=agState.getenergiaTotal())){//Si el nodo esta habilitado puede avanzar
	    			...
	    			agState.setenergiaConsumida((int)(agState.getenergiaConsumida()+this.getCost(nodoActual.getY(),n.getY())));
	    			return agState;
				}
			}
    	}
        ...
        for(Nodo n : nodosAdyacentes){
    		if((nodoActual.getX()==n.getX())&&(nodoActual.getY()>n.getY())&&(!n.equal(agState.getUltimaPosicionVisitada()))){
				...
				if(!estaBloqueado){//Si el nodo esta habilitado puede avanzar
	    			...
	    			environmentState.setEnergiaConsumida((int)(environmentState.getEnergiaConsumida()+this.getCost(nodoActual.getY(),n.getY())));
